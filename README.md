...
Written for the standardized item-analysis workflow (the 2025 tryout runs: Kuantitatif, Verbal, Penalaran, Pemecahan
...
**Working end to end and verified column by column against official Winsteps output on six real tryout runs.**
The reference baselines themselves are not distributed because they derive from a client dataset, so those regression checks skip unless you point `RASCHLAB_DATA_DIR` at your own reference directory.
...
| Item label column (`ITEM`, from the `ILABEL` file) on the item table | done, verified: item labels identical to the reference's TABLE 15.1 |
...
| Reference convergence rules (LCONV/RCONV, PROX 0.5-logit range rule) verified by regression tests | done -- `tests/regression/test_golden_convergence.py` |
| Formula-level parity audit (every formula/convention, with status and evidence) | done -- `docs/parity.md` |
| Tests | passing (reference-dependent checks skip when reference data is absent) |
...
|---|---|---|---|---|---|
| kuantitatif | hundreds | 3 | ~2,300 | **0.0226** | 0.0313 |
| verbal | dozens | 1 | ~2,000 | **0.0285** | 0.0523 |
| penalaran | hundreds | 2 | ~2,200 | **0.0105** | 0.1153 |
| pemecahan | dozens | 4 | ~2,300 | **0.0251** | 0.0271 |
| penalaran_rev | hundreds | 2 | ~2,200 | **0.0090** | 0.1133 |
| pemecahan_rev | dozens | 4 | ~2,300 | **0.0314** | 0.0357 |
...
the `MISSING ***` row (about two thousand respondents).
...
Both rules are verified by `tests/regression/test_golden_convergence.py` (which skips when reference baselines are withheld).
...
Measured against the reference outputs across hundreds of items over six real tryout runs, worst case per column, with the shipped
`--lconv 0.015`:
...
- Threshold sensitivity: `0.0125` (the previous default) was 3-5x worse on the penalaran runs -- `INFIT ZSTD`
...
The three former P1 gaps are implemented and verified against the reference tables (see *Conventions* and *Known
...
7. Unanchored estimation: `tests/regression/compare_unanchored.py` compares our anchor-free estimates against
   the free measures implied by the reference `DISPLACE` column (15 anchored items over five runs). It gates at
...
9. CI: deliberately skipped (12 Sep 2026). A ready-to-use workflow was written and tested locally --
   ...
   (passing, reference-dependent checks skip without the reference data) -- but the repository token only carries `repo`, `gist`
...
python -m raschlab analyze \
  --con /tmp/raschlab-reference/cfile_kuantitatif.CON \
  --data /tmp/raschlab-reference/kuantitatif_data.prn \
  --anchors /tmp/raschlab-reference/iafile_kuantitatif.TXT \
  --pdfile /tmp/raschlab-reference/pdfile_kuantitatif.TXT \
...
```bash
python -m raschlab analyze-all --dir /tmp/raschlab-reference --out /tmp/raschlab_batch --mode compat --format csv
```
...
```text
kuantitatif: items=150 persons_reported=2300 iterations=3 elapsed=1.04s
pemecahan: items=75 persons_reported=2300 iterations=6 elapsed=0.65s
pemecahan_rev: items=75 persons_reported=2300 iterations=6 elapsed=1.82s
penalaran: items=100 persons_reported=2200 iterations=14 elapsed=1.60s
penalaran_rev: items=100 persons_reported=2200 iterations=15 elapsed=0.75s
verbal: items=50 persons_reported=2200 iterations=12 elapsed=0.67s
```
...
```bash
RASCHLAB_DATA_DIR=/tmp/raschlab-reference .venv/bin/python tests/regression/compare_all_runs.py
RASCHLAB_DATA_DIR=/tmp/raschlab-reference .venv/bin/python -m pytest tests -q
```
...
- [`docs/parity.md`](docs/parity.md) -- formula and convention audit against the reference tool.
