...
.venv/bin/python -m pytest -q                                                                # 94 tests, 5 skipped without the reference data
...
2. **The item label column was a real parity gap, not a nicety.** The reference's Table 15.1 carries an `ITEM`
   ...
   item table, appended after the 13 existing columns so no consumer breaks; all 147 labels match the reference
...
3. **BIGSTEPS v2.82 (the DOS ancestor of Winsteps, Linacre & Wright 1998) was run headless under DOSBox** as a
   ...
   reference. On 300 persons × 147 items with 87% of the matrix missing it reproduced this tool's item measures
   to max |diff| 0.05 logit, with `SCORE` identical on all 147 items. On the full run the two disagree only at
   the three anchored items (46, 92, 147), which is exactly what an un-anchored run should look like next to an
